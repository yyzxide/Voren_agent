...
- implementation revision: `1839c3e`;
- clean locked installation: CPython 3.12 on Linux x86-64 using `pylock.toml`;
- local result: 185 of 185 unit and integration tests passed;
- remote result: [GitHub Actions run 34815893634](https://github.com/yyzxide/Voren_agent/actions/runs/34815893634) passed;
...
- one dated live-model golden run and prompt-injection evaluation artifact;
- one dated live-model `no_skill` versus Skill comparison;
- one redacted smoke artifact from a dedicated Google test account.

These are execution/evidence gates, not hidden automated-test claims. Until they
exist, the repository does **not** claim live-model planning quality, general
prompt-injection resistance, or successful access to a real Google account.
